...
export default function CartPage() {
  ...
  // Read promo state client-side only (avoids SSR mismatch)
          ...
          <div className="border border-stone-100 bg-white px-6 py-8">
            ...
            {savedPromoCode && (
              <p className="text-xs text-stone-500 mb-5">
                Code <span className="font-mono font-semibold tracking-wide">{savedPromoCode}</span> saved — apply at checkout.
              </p>
            )}
            ...
            {/* Total */}
            <div className="flex w-full items-center justify-between mt-4 pt-4 border-t border-stone-200">
              <span style={{ fontSize: 15 }} className="font-medium text-stone-900">Total</span>
              <span style={{ fontSize: 17 }} className="font-semibold text-stone-900">Due at checkout</span>
            </div>

            {/* CTA */}
            <Link
              href="/checkout"
              className="mt-6 block w-full rounded-full bg-stone-900 py-4 text-center text-sm font-medium tracking-wide text-white transition-colors hover:bg-stone-700"
            >
              Proceed to Checkout
            </Link>
